...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!ai.ok) {
      return res.status(ai.status || 500).json(ai.body || { error: 'Falha ao chamar IA' });
    }

    let jsonStr = String(ai.result || '').trim();
    ...
    return res.status(200).json({ evaluation: parsed, raw: ai.result });
  } catch (err) {
